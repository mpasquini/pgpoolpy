...
class LockFileBusy(LockFileException):
    """
    Raised when a lock fd is not free
    """
    pass
...
class LockFilePermissionDenied(LockFileException):
    """
    Raised when a lock fd is not accessible
    """
    pass
...
class LockFile(object):
    ...
    statements.
        ...
        with LockFile('fd.lock') as locked:
            if not locked:
                print "failed"
            else:
                <do something>
    ...
    You can also use exceptions on failures
        ...
        try:
            with LockFile('fd.lock', True):
                <do something>
        except LockFileBusy, e, fd:
            print "failed to lock %s" % fd
    ...
    def acquire(self, raise_if_fail=None, wait=None):
        """
        Creates and holds on to the lock fd.
        ...
        try:
            ...
            # Once locked, replace the content of the fd
            os.lseek(fd, 0, os.SEEK_SET)
            os.write(fd, ("%s\n" % os.getpid()).encode('ascii'))
            # Truncate the fd at the current position
            os.ftruncate(fd, os.lseek(fd, 0, os.SEEK_CUR))
            self.fd = fd
            return True
        except (OSError, IOError) as e:
            if fd:
                os.close(fd)  # let's not leak  fd descriptors
            if raise_if_fail:
...
class GlobalCronLock(LockFile):
    ...
    Creates a global '.cron.lock' lock fd under the given lock_directory.
    """
...
#
#     Creates a '.<SERVER>-backup.lock' lock fd under the given lock_directory
...
#
#     Creates a '.<SERVER>-cron.lock' lock fd under the given lock_directory
#     for the named SERVER.
#     """
#
#     def __init__(self, lock_directory, server_name):
...
#
#     Creates a '.<SERVER>-xlogdb.lock' lock fd under the given lock_directory
#     for the named SERVER.
#     """
#
#     def __init__(self, lock_directory, server_name):
...
#
#     Creates a '.<SERVER>-archive-wal.lock' lock fd under
...
#
#     Creates a '.<SERVER>-receive-wal.lock' lock fd under
#     the given lock_directory for the named SERVER.
#     """
